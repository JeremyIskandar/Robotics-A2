function [collision_point] = myLightCurtainCollision(light_curtain_points,object_points)
    ...
    tolerance = 0.05;                                           % Set distance tolerance to 0.02
    for lc = 1:length(light_curtain_points)
        ...
        for obj = 1:length(object_points)
            obj_x = object_points(obj,1);
            obj_y = object_points(obj,2);
            obj_z = object_points(obj,3);
            dist_xz = sqrt((obj_x-lc_x)^2+(obj_z-lc_z)^2);      % Calculate the distance between x and z points
            dist_xy = sqrt((obj_x-lc_x)^2+(obj_y-lc_y)^2);      % Calculate the distance between x and y points
            ...
            if (dist_xz < tolerance) && (dist_xy < tolerance) && (flag == 0)
                ...
                plot3(lc_x,lc_y,lc_z,'*','Color','r','MarkerSize',12,'LineWidth',2);    % Plot the collision point in red in the environment
            end
        end
    end
