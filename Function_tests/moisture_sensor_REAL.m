function [] = moisture_sensor_REAL(dobot)
    ...
    plant_moisture_levels = randsample(range,3);                % Develops random number for moisture values between range
    ...
    if size_LI(2) > 0
        for x = 1:1:size_LI(2)
            ...
            disp(y);
            water_plant_real_REAL(which_plant,dobot);
        end
    else
